...
大部分人以为 AI 味是用词问题——"值得注意的是"、"总的来说"、破折号、数字列表。这是表面症状。

**真正的问题是模式**：

1. **AI 每段都整整齐齐地"开头-展开-总结"三段式** — 人类写作有跳脱、有省略、有突然的转折
2. **缺乏观点立场** — AI 默认对所有话题两面讨好，人类有自己的偏见
3. **所有回复的结构如出一辙** — 读者读两篇就能识别出"这是同一个人写的"，且这个人不是真人
4. **结尾总是回归到训练数据的统计平均值** — 总结、展望、鼓励

> **所以去AI味的核心不是换词，是打破模式。** 词汇替换只是表面，真正需要的是在结构、节奏、观点密度上刻意偏离 AI 的"舒适区"。

## 快速八项检查（发布前必检）

这 8 条比下面的详细清单更快。**任何一条不合格 → 修改后再发**：

1. □ 有没有「值得注意的是」「总的来说」「此外」等套话？→ 删
2. □ 每段是否都整齐「开头-展开-总结」？→ 打破结构
3. □ 连续数字列表超过 2 个？→ 改成叙述
4. □ 结尾是泛泛总结或展望？→ 换掉（参考黄金结尾部分）
5. □ 缺少「我」的视角？→ 加真实故事或个人观点
6. □ 所有观点都平衡中立？→ 大胆表态
7. □ 读出声，哪里走神了？→ 那里要改
8. □ 删掉最后一段，文章更好？→ 直接删

> **快速检验法**：AI 写的文章，直接删掉最后一个总结段，通常倒数第二段反而更有力。
...
- **模糊背书（"worth [verb]ing"）**：删掉或替换 `worth reading`, `worth paying attention to`, `worth a look`, `worth exploring`, `worth checking out`, `worth your time`。说*为什么*某事重要。
...
- "That said" / "That being said" → 删掉或用 "but," "yet," 或 "however"

### 4. 系动词回避
- AI文本通过用更华丽的动词替换来回避 "is" 和 "has"："serves as," "features," "boasts," "presents," "represents"。
- 默认使用 "is" 或 "has"，除非更具体的动词真正增加了意义。

### 5. 同义词循环
- AI在同一自然段中轮换同义词以避免重复一个词。人类作者会重复最清晰的词。
- 如果一个名词或动词在段落中出现三次且那是正确的词，保留全部三个。
...
### 7. 聊天机器人痕迹
- "I hope this helps!", "Certainly!", "Great question!", "Feel free to reach out" — 完全删除。
- "In this article, we will explore…" 或 "Let's dive in!" — 删掉或用直接开头重写。

### 8. "Let's" 结构
- "Let's explore," "Let's take a look," "Let's break this down" — 直接从要点开始。

### 9. 模糊归因
- "Experts believe," "Studies show," "Research suggests" — 要么引用具体来源，要么删掉。

### 10. 重要性夸大
- "marking a pivotal moment" / "a watershed moment" — 陈述发生了什么，让读者判断。

### 11. 通用结论
- "The future looks bright," "Only time will tell," "One thing is certain" — 删掉。

### 12. 情绪和节奏问题
- **段落长度统一**：有意变化。包括一些1-2句话的段落和一些较长的。
- **句子长度统一**：混合短促有力的句子（3-8字）和较长的流畅句子（20+字）。
- **过度润色**：自然的不流畅、独特的词选择和不均匀的节奏是让文本不出现在"AI生成"分类中的原因。
