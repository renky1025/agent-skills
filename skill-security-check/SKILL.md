...
需要访问用户指定服务地址的 Skill，应在 `SKILL.md` frontmatter 中明确声明：

```yaml
network_access: user-configured-origin
```

该声明只把网络请求从“未声明外发”降为“需要用户知晓并人工审查”，不会自动证明安全。审查时仍须确认代码只使用用户配置的 base URL，不包含固定第三方上报端点、隐蔽 telemetry 或跨主机重定向。
